...
const Chatbot = () => {
  ...
  return (
    <>
      {/* Chatbot Button */}
      <div
        className={`fixed bottom-20 right-6 z-50 w-14 h-14 flex justify-center items-center bg-black dark:bg-white dark:text-white text-black shadow-lg cursor-pointer transition-all duration-300 transform  rounded-full ${isOpen ? 'rotate-180' : ''
          }`}
        onClick={toggleChat}
      >
        ...
        <div className="fixed bottom-36 right-6 sm:right-8 w-80 sm:w-[300px] bg-white dark:bg-gray-800 p-4 rounded-lg shadow-lg border border-gray-300 dark:border-gray-600 flex flex-col h-[500px] sm:h-[400px]">
          ...
          <div className="mt-4 flex-grow overflow-y-auto max-h-[60%] sm:max-h-[70%] pr-2 custom-scrollbar">
            {messages.map((msg, index) => (
              ...
              >
                <div
                  className={`p-2 max-w-[70%] rounded-lg text-sm ${msg.sender === 'user'
          ...
          <div className="mt-2 flex flex-col items-center justify-center gap-2">
            <Textarea
              className="w-[80%] border-2 border-black dark:border-white mx-auto p-2 rounded-md text-sm dark:text-white dark:caret-white caret-black text-black focus-visible:ring-0 focus-visible:outline-none"
